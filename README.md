...
## 另附：BOW 封閉式電商 mock-up（`/demo/bow`）

給 BOW 提案用的可點擊介面模擬，與地圖分析無關，只是共用同一個 Next.js app 部署。

情境：專櫃銷售人員把「專屬選品連結」傳給自己的熟客 → 顧客在 BOW App 完成付款 →
訂單自動掛回該員編。用意是把 Moana 的線上聲量一路量到實際成交金額，而不是停在曝光數。

- 流程參考 `新光三越 熟客推薦系統` 簡報（同仁登入 → 商品建檔 → 分享連結 → 顧客下單 → 訂單狀態）。
- 三條動線：專櫃銷售（5 頁）／顧客（6 頁）／總部歸因儀表板（2 頁）。
- 右上角可切 **EN／繁中**（說明文字）與 **逐步導覽／全部畫面**；「全部畫面」是為了方便直接截圖進簡報。
- 資料全部是 `web/src/lib/data/bow.ts` 裡的假資料；品牌名（Maison Aurel、Céleste、Rivière）為虛構佔位。
